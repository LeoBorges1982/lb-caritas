...
export default function BotaoExcluirLancamento({ id, descricao }: { id: string; descricao?: string }) {
  const [excluindo, setExcluindo] = useState(false);

  async function onClick(e: React.MouseEvent) {
    e.preventDefault();
    e.stopPropagation();
    const msg = descricao ? `Excluir lançamento "${descricao}"?` : "Excluir esse lançamento?";
